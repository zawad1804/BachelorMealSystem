...
$payments_query = "SELECT p.*, 
                  ...
                  ORDER BY p.payment_date DESC";  // Fixed column name from p.date to p.payment_date
$payments_result = $conn->query($payments_query);

// Replace the existing unpaid members query with this:
$unpaid_members_query = "SELECT DISTINCT u.id, u.first_name, u.last_name, u.balance 
                        FROM users u 
                        LEFT JOIN (
                            SELECT user_id 
                            FROM payments 
                        ...
                        ) today_payment ON u.id = today_payment.user_id
                        WHERE u.role = 'member' 
                        AND today_payment.user_id IS NULL
                        ORDER BY u.first_name";

// Add error handling
if (!$unpaid_members_result = $conn->query($unpaid_members_query)) {
    $_SESSION['error'] = "Error fetching unpaid members: " . $conn->error;
}

// Add this after the query for debugging
if ($unpaid_members_result) {
    echo "<!-- Number of unpaid members: " . $unpaid_members_result->num_rows . " -->";
} else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'approve') {
            ...
            // Update user balance
            $update_balance = "UPDATE users SET balance = 
                (SELECT total_balance FROM (
                    SELECT (balance - ?) as total_balance 
                    FROM users WHERE id = ?
                ) AS temp) 
                WHERE id = ?";
            $stmt = $conn->prepare($update_balance);
            $stmt->bind_param("dii", $amount, $user_id, $user_id);
            $stmt->execute();
...
<body>
    ...
    <main class="admin-container">
        ...
        <section class="glass-card">
            <h2>Members Without Payment Today</h2>
            <table class="payments-table">
                <thead>
                    <tr>
                        <th>Member Name</th>
                ...
                <tbody>
                    <?php if ($unpaid_members_result && $unpaid_members_result->num_rows > 0): ?>
                        ...
                        <?php endwhile; ?>
                    <?php else: ?>
                        <tr><td colspan="4">All members have paid this month!</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </section>
